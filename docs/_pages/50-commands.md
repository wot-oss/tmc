...
Depending on how AWS client access is configured where the `tmc` binary is running (e.g. via IAM role or shared config and credential files), additional fields may be necessary for the S3 repo.
...
E.g. for an HTTP repo served by a GitLab installation, you may use `https://example.com/api/v4/projects/<project-id>/repository/files/{% raw %}{{ID}}{% endraw %}?ref=main` as the URL.

### TMC Repositories

TMC repo allows you to access a remotely hosted repository via the TMC REST API, which is provided by `tmc serve`.
...
name "README.md" and attach it to the corresponding TM id. Given this setup, if you then execute `tmc attachment fetch --concat <tm-name> README.md`
...
those that were added by using `tmc`. An exception to that is that files on the top level and directories starting with a dot
are always ignored. In fact, when you run `tmc check` for the first time, a `.tmcignore` file with defaults is created
in the `.tmc` folder under the file repository's root. If you do want to store some other files along with TMs and attachments
...
- share a complete TMC setup as a containerized application
